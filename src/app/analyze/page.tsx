      ...
      <div className="w-full max-w-md space-y-8">
        ...
        {result && (
          <div className="p-8 bg-white/5 border border-white/10 rounded-3xl text-center mt-4">
            <p className="text-2xl font-bold mb-2">{result.reframe}</p>
            <p className="text-sm text-gray-400 mb-6">{result.biasType}</p>
            <button
              onClick={handleDownload}
              className="w-full bg-violet-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-violet-500 transition-colors"
            >
              Gorseli Indir
            </button>
            <p className="text-xs text-gray-500 mt-3">
              Gorseli indirip Instagram Hikayelere yukleyebilirsin
            </p>
          </div>
        )}
      </div>
